// Recruiter version of Notification Service (mostly identical to candidate version)
// ========================= Notification Service for Job Portal (Recruiter) =========================
...
// Global instance for recruiter pages
const recruiterNotificationService = new RecruiterNotificationService();
let recruiterNotifications = [];

function hasEl(id) { return !!document.getElementById(id); }
function onRecruiterListPage() { return hasEl('notifications-content') && hasEl('notifications-list'); }
...
async function loadNotifications() {
  ...
  try {
    ...
    const success = await recruiterNotificationService.loadNotifications();
    if (!success) { showError(); return; }

    recruiterNotifications = recruiterNotificationService.getAllNotifications();
    displayNotifications();
    updateNotificationCount();
    hideLoading();

    recruiterNotificationService.updateBadge?.();
  } catch (err) {
    console.error('Error loading notifications:', err);
    showError();
  }
...
function displayNotifications() {
  ...
  if (!Array.isArray(recruiterNotifications) || recruiterNotifications.length === 0) {
    showEmptyState();
    return;
  }

  const html = recruiterNotifications.map(n => {
    const body = n?.body || '';
    return `
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 ${!n.readAt ? 'border-l-4 border-l-blue-500' : ''} hover:shadow-md transition-shadow cursor-pointer"
           onclick="goToRecruiterNotificationDetail(${n.id})">
...
function updateNotificationCount() {
  ...
  const unread = recruiterNotifications.filter(n => !n.readAt).length;
  const total = recruiterNotifications.length;
...
async function markAsRead(notificationId) {
  try {
    const success = await recruiterNotificationService.markAsRead(notificationId);
...
async function confirmMarkAllRead() {
  try {
    closeMarkReadModal();
    const success = await recruiterNotificationService.markAllAsRead();
...
function goToRecruiterNotificationDetail(notificationId) {
  window.location.href = `recruiter-notification-detail.html?id=${notificationId}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    if (typeof authService !== 'undefined') {
      recruiterNotificationService.initialize();
    }
  }, 100);

  if (onRecruiterListPage()) {
...
window.goToRecruiterNotificationDetail = window.goToRecruiterNotificationDetail || goToRecruiterNotificationDetail;